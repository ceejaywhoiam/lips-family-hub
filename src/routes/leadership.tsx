...
export const Route = createFileRoute("/leadership")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Meet the placeholder leadership team guiding LIPS Family and its creator agency.",
      },
...
const leaders = [
  {
    name: "Lola Baby",
    position: "CEO of LIPS Family",
    image: lolaBaby,
    instagram: "@lolababy_streams",
    placeholder: false,
  },
  {
    name: "Jodi Please",
    ...
    image: jodiPlease,
    instagram: "@jodiplease",
    placeholder: false,
  },
  {
    name: "Peppermint",
    position: "Head of Conflict Resolutions",
    image: peppermint.url,
    instagram: "@_crypticsilences_",
    placeholder: false,
  },
  {
    name: "Kai Osei",
...
function LeadershipPage() {
  return (
    <PageShell
      eyebrow="The crew"
      title="Leadership"
      intro="The people shaping the LIPS vision, supporting talent, and building a creator-first culture."
    >
      <div className="grid gap-5 md:grid-cols-2">
        {leaders.map((leader) => (
          <article
            key={leader.name}
            className="glass-panel overflow-hidden rounded-2xl border border-border flex flex-col md:grid md:grid-cols-[0.85fr_1.15fr]"
          >
            ...
            <div className="flex flex-col justify-between p-5 md:p-7">
              <img
                src={logo.url}
                alt="LIPS Family crown logo"
                width={768}
                height={768}
                loading="lazy"
                className="mb-10 size-20 rounded-md border border-border object-cover"
              />
              <div>
                ...
                {leader.placeholder && (
                  <p className="mt-2 text-[10px] italic text-muted-foreground">
                    Placeholder entry — swap in the real name, position, photo, and Instagram.
                  </p>
                )}
              </div>
            </div>
