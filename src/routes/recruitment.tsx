...
export const Route = createFileRoute("/recruitment")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Apply to join the LIPS creator family or the LIPS talent agency." },
...
function RecruitmentPage() {
  ...
  return (
    <PageShell eyebrow="Applications open" title="Choose your path." intro="Apply for representation through the agency or join the creator family. Select one path to begin.">
      <div className="mx-auto max-w-3xl">
        ...
        <form onSubmit={submit} className="glass-panel space-y-5 rounded-2xl border border-border p-5 md:p-8">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-primary">Selected path</p>
            <h2 className="mt-1 font-display text-2xl font-black text-foreground">Join the {path === "agency" ? "Agency" : "Family"}</h2>
          </div>
          {path === "family" ? (
            <>
              <Field id="experience" name="experience" label="How long have you been streaming?" placeholder="For example: 2 years" required maxLength={100} />
              <TextField id="content" name="content" label="What kind of content do you create?" placeholder="Tell us about your content and audience" required maxLength={800} />
              <TextField id="contribution" name="contribution" label="What can you bring to the family?" help="Share what makes you stand out and how you could help the brand." placeholder="Your strengths, ideas, and unique perspective" required maxLength={1200} />
              <Field id="platforms" name="platforms" label="Streaming platforms you use" placeholder="Twitch, YouTube, Kick…" required maxLength={200} />
            </>
          ) : (
            <Field id="platform" name="platform" label="Streaming platform you use" placeholder="For example: Twitch" required maxLength={100} />
          )}
